...
pidfile=pid_recommend_score

cwd=`pwd`
appname='recommand-score'
...
start()
{
    if [ -e $pidfile ] 
    then
        ps -fs `cat $pidfile` | grep recommend-score > /dev/null 2>&1
    	if [ $? -eq 0 ]
	    then
...
stop()
{
    ps aux | grep php | grep recommend-score | grep -v php-fpm  | awk '{ print $2}' | xargs kill -9
}
...
state()
{
    ps -fs `cat $pidfile` | grep "recommend-score"
    running=$?
    if [ $running -ne 0 ]
    then
