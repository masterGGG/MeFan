...
pidfile=pid_recommend_process

cwd=`pwd`
appname='recommand-process'
...
start()
{
    if [ -e $pidfile ] 
    then
        ps -fs `cat $pidfile` | grep recommend-process > /dev/null 2>&1
    	if [ $? -eq 0 ]
	    then
...
stop()
{
    ps aux | grep php | grep recommend-process | grep -v php-fpm  | awk '{ print $2}' | xargs kill -9
}
...
state()
{
    ps -fs `cat $pidfile` | grep "recommend-process"
    running=$?
    if [ $running -ne 0 ]
    then
